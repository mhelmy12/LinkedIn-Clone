...
namespace APIGateway.UserModule;
...
internal static partial class UserModule
{
    internal static IEndpointRouteBuilder MapUserEndpoints(this IEndpointRouteBuilder builder)
    {
            ...
            (ClaimsPrincipal principal) =>
            {
                var user = principal switch
                {
                    { Identity.IsAuthenticated: true } => new User
                    {
                        ...
                        Claims = principal.Claims.Select(c => new UserClaim
                        {
                            Type = c.Type,
                            Value = c.Value,
                        }),
        ...
        builder.MapGet(
            "login",
            (string? returnUrl, string? claimsChallenge, HttpContext context) =>
            {
        ...
        builder.MapGet(
            "logout",
            (string? redirectUrl, HttpContext context) =>
            {
                var properties = new AuthenticationProperties
                {
                    RedirectUri = context.BuildRedirectUrl(redirectUrl, true),
                };
