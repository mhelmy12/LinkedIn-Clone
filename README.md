...
flowchart LR
    ...
    User --> UserDB[(User PostgreSQL Database)]
    Post --> PostDB[(Post PostgreSQL Database)]
    Engagement --> EngagementDB[(Engagement PostgreSQL Database)]
    Feed --> FeedDB[(Feed PostgreSQL Database)]
    Feed --> Redis[(Redis)]
...
3. Each service persists transactional data in its own PostgreSQL database using EF Core migrations.
...
| Relational data      | PostgreSQL            | Transactional data for users, posts, reactions, comments, feeds, and Keycloak  |
...
Application events describe business intent; database changes describe committed state. Debezium captures committed row-level changes from PostgreSQL through the WAL log and publishes them to Kafka.
...
### Why Elasticsearch instead of searching directly in PostgreSQL?
...
PostgreSQL remains the source of truth. Elasticsearch can be rebuilt from Kafka events, so search availability and indexing speed do not need to dictate the transactional schema.
...
Users, connections, posts, reactions, comments, and their relationships require transactions, constraints, indexes, and predictable consistency. PostgreSQL and EF Core provide those capabilities with mature migrations, tooling, and operational support.
...
The current local setup uses PostgreSQL with persistent volumes and one database per service-owned area. Connection strings are supplied through configuration, and EF Core manages connections through the underlying ADO.NET pool.

As the number of services and replicas grows, the database will eventually need a connection-management strategy that protects PostgreSQL from connection storms. The planned direction is:
...
The repository currently uses configuration sections for service URLs, PostgreSQL connections, Kafka bootstrap servers, Elasticsearch, Keycloak, Redis, and S3-compatible storage. Check each service's `appsettings.Development.json` before starting it.
...
- High-availability PostgreSQL and tested backup/restore procedures.
...
- Add automated integration tests for PostgreSQL, Kafka, Elasticsearch, Redis, and MinIO flows.
